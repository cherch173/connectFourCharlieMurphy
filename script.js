...
function renderMessage() {
  /// IT'S A TIE message
  if (winner === 'T') {
    messageEl.innerHTML = `It's a <span style="color: rebeccapurple">draw.</span> <br><span style="font-size: 5vmin"><span style="color: cadetblue">Purify yourself </span>in the waters of <span style="color: cadetblue"> Lake Minnetonka.</span></span></br>`;
  }
  /// PLAYER 2 WINS message
  else if (winner <= '-1') {
    messageEl.innerHTML = `...game, Blouses. <span style="color: ${COLORS[winner]}">  Prince </span> wins.`;
  }
  /// PLAYER 1 WINS message
  else if (winner >= '1') {
    messageEl.innerHTML = `Congratulations, <span style="color: ${COLORS[winner]}">  Charlie Murphy. </span> You win. <br>...<span style="font-size: 5vmin"><span style="color: ${COLORS[winner]}">pancakes </span>will be served momentarily.</span></br>`;
  }
